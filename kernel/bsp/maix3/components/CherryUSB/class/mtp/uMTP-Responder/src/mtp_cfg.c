...
int mtp_load_config_file(mtp_ctx * context, const char * conffile)
{
	...
	{
		PRINT_DEBUG("Can't open %s ! Using default settings...", conffile);
	}

	PRINT_DEBUG("USB Max packet size : 0x%X bytes",context->usb_cfg.usb_max_packet_size);
	...
	PRINT_DEBUG("Serial string : %s",context->usb_cfg.usb_string_serial);
	PRINT_DEBUG("Firmware Version string : %s", context->usb_cfg.usb_string_version);

	PRINT_DEBUG("USB Vendor ID : 0x%.4X",context->usb_cfg.usb_vendor_id);
	PRINT_DEBUG("USB Product ID : 0x%.4X",context->usb_cfg.usb_product_id);
	PRINT_DEBUG("USB class ID : 0x%.2X",context->usb_cfg.usb_class);
	PRINT_DEBUG("USB subclass ID : 0x%.2X",context->usb_cfg.usb_subclass);
	PRINT_DEBUG("USB Protocol ID : 0x%.2X",context->usb_cfg.usb_protocol);
	PRINT_DEBUG("USB Device version : 0x%.4X",context->usb_cfg.usb_dev_version);

	if(context->usb_cfg.usb_functionfs_mode)
	{
		PRINT_DEBUG("USB FunctionFS Mode");
	}
	else
	{
		PRINT_DEBUG("USB GadgetFS Mode");
	}

	PRINT_DEBUG("Wait for connection : %i",context->usb_cfg.wait_connection);
	PRINT_DEBUG("Loop on disconnect : %i",context->usb_cfg.loop_on_disconnect);
	PRINT_DEBUG("Show hidden files : %i",context->usb_cfg.show_hidden_files);
	if(context->usb_cfg.val_umask >= 0)
	{
		PRINT_DEBUG("File creation umask : %03o",context->usb_cfg.val_umask);
	}
	else
	{
		PRINT_DEBUG("File creation umask : System default umask");
	}

	if(context->default_uid != -1)
	{
		PRINT_DEBUG("Default UID : %d",context->default_uid);
	}
	else
	{
		PRINT_DEBUG("Default UID : %d",geteuid());
	}

	if(context->default_gid != -1)
	{
		PRINT_DEBUG("Default GID : %d",context->default_gid);
	}
	else
	{
		PRINT_DEBUG("Default GID : %d",getegid());
	}

	PRINT_DEBUG("inotify : %s",context->no_inotify?"no":"yes");
