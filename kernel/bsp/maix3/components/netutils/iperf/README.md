...
iperf 使用客户端/服务器架构，服务器和客户端模式均支持 TCP 和 UDP。命令选项可以按任意顺序提供；`-i` 设置报告间隔，`-t` 设置测试时长，`-P` 设置并行客户端线程数。UDP 客户端默认逐步提高限速速率，并根据驱动反压确定最大稳定传输速率；`-b` 可以使用每秒比特数或 `K`、`M`、`G` 后缀设置固定负载。
...
- -p 表示连接 5001 端口
- -b 设置 UDP 发送负载；应将服务器端接收速率和丢包数与客户端发送负载对比
- 等待测试结束。测试时，测试数据会在 shell 界面和 JPerf 软件上显示。
