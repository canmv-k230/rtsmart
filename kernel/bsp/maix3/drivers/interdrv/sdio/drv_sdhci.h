...
/* Preserve card interrupts during bulk status cleanup so a command transfer
 * cannot discard a notification before the IRQ handler queues it. The IRQ
 * handler acknowledges the bit after masking the host interrupt. */
#define SDHCI_INT_ACK_MASK (SDHCI_INT_ALL_MASK & ~SDHCI_INT_CARD_INT)
