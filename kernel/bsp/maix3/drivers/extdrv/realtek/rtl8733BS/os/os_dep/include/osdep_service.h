...
#define CONFIG_LITTLE_ENDIAN 1				//= 1 for little-endian(default)
#endif
...
typedef uint64_t clock_s;

struct work_s {
	struct dq_entry_s dq;		/* Implements a doubly linked list */
	worker_t worker;			/* Work callback */
	FAR void *arg;				/* Callback argument */
	clock_s qtime;			/* Time work queued */
	clock_s delay;			/* Delay until work performed */
};
...
#if defined(CONFIG_THREAD_COMM_SEMA)
 ...
 * @return	  None
 */
void	rtw_memcpy(void* dst, const void* src, u32 sz);
...
#else
u32 rtw_timerDelete( _timer *timer, osdepTickType xBlockTime );
#endif
/**
