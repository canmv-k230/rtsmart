...
/** 
 * @brief  enable multicast address filter.\n
 * 		if enabled, Once a multicast address has been unregistered, all packets detected on the
 * 		medium destined for that address are ignored.
 * @return  RTW_SUCCESS: If the address is unregistered successfully.
 * @return  RTW_ERROR: If the address is not unregistered.
 */
...
/** 
 * @brief  disable multicast address filter.\n
 * 		if disabled, will recv all multicast data
 * @return  RTW_SUCCESS: If the address is unregistered successfully.
 * @return  RTW_ERROR: If the address is not unregistered.
 */
...
/**
 * @brief  Set Beacon Mode.
 * @param[in] mode: 1 means Enable, 0 means Disable
 * @return  RTW_SUCCESS if set Beacon Mode successful.
 * @return  RTW_ERROR otherwise.
 */
...
/**
 * @brief  enable adaptivity
 * @return  RTW_SUCCESS
 */
int wifi_enable_adaptivity(void);

/**
 * @brief  disable adaptivity
 * @return  RTW_SUCCESS
 */
...
/*\@}*/
#if CONFIG_RW_PHYSICAL_EFUSE && RTW_HALMAC
...
int wifi_set_scan_reorderchan(__u8 * channel_list, __u8 length);
