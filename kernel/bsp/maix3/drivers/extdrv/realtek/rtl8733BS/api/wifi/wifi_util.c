...
extern int rltk_get_txpower(const char *ifname, u8* txpoweridx);
extern int rltk_get_max_support_rate(const char *ifname, u8 *prate,u8 *pfixed);
...
/*
prate =0x00 means set data frame packet data rate = fixed 1Mbps(DESC_RATE1M)
	  =0x01 means set data frame packet data rate = fixed 2Mbps(DESC_RATE2M)
	  ...
	  =0xff means restore default setting of auto rate
*/
void realtek_wl_set_bit_rate()
{
    char* ifname = "wlan0";
    unsigned char prate=0x00;
    char cmd[14];
...
extern int rltk_set_traffic_busy_thres(unsigned int traffic_busy_thres);
int wext_set_traffic_busy_thres(unsigned int traffic_busy_thres)
{
	return rltk_set_traffic_busy_thres(traffic_busy_thres);
}
...
extern int rltk_set_wap_mode(const char *ifname,rtw_wpa_mode wpa_mode);
int wext_set_wap_mode(const char *ifname,rtw_wpa_mode wpa_mode)
{
	return rltk_set_wap_mode(ifname,wpa_mode);
}
