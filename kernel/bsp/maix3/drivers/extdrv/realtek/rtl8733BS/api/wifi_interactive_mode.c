/* Ensure CONFIG_PLATFOMR_CUSTOMER_RTOS is visible for conditional paths. */
#include "autoconf.h"
...
/*
After wifi_on
Input :		wifi_set_fix_rate 20
			to set fix_rate = 20 = 0x14 (DESC_RATEMCS8)
Then input: iwpriv dbg phy
			to see Fixed rate = 20
*/
static void cmd_wifi_set_fix_rate(int argc, char **argv)
{
    char* ifname = "wlan0";
    char cmd[14];
    int prate = atoi((const char *)argv[1]);
...
#if defined (STM32L4R5xx) || defined (STM32U5A5xx)
extern void HAL_NVIC_SystemReset(void);
static void cmd_software_reset(int argc, char **argv)
{
...
#endif	
	{"wifi_set_fix_rate", cmd_wifi_set_fix_rate},
