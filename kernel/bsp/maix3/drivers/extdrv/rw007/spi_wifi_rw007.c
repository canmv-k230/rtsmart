...
static int wifi_data_transfer(struct rw007_spi *dev, uint16_t seq, uint8_t *rx_buffer, const void *tx_buffer)
{
    ...
    struct rt_qspi_device *rt_spi_device = dev->spi_device;
    ...
    message.parent.send_buf = &cmd;
    message.parent.recv_buf = &resp;
    message.parent.length = sizeof(resp);
    message.parent.cs_take = 1;
    message.parent.cs_release = 1;
    message.parent.next = RT_NULL;

    rt_qspi_transfer_message(rt_spi_device, &message);
    /* Clear event */
    ...
    message.parent.send_buf = send_packet;
    message.parent.recv_buf = rx_buffer;
    message.parent.length = RT_ALIGN(max_data_len, 4);/* align clk to word */
...
_txerr:
    ...
    message.parent.length =0;/* align clk to word */
    message.parent.cs_take = 0;
    message.parent.cs_release = 1;
    rt_qspi_transfer_message(rt_spi_device, &message);
...
static void parse_scan_data(struct rt_wlan_info *dst, const rt_uint8_t *src)
{
    const struct kd_wlan_info *raw = (const struct kd_wlan_info *)src;

    dst->security = raw->security;
    dst->band     = raw->band;
    dst->datarate = raw->datarate;
    dst->channel  = raw->channel;
    dst->rssi     = raw->rssi;

    dst->ssid.len = raw->ssid.len > RT_WLAN_SSID_MAX_LENGTH ?
                    RT_WLAN_SSID_MAX_LENGTH : raw->ssid.len;
    rt_memcpy(dst->ssid.val, raw->ssid.val, dst->ssid.len);
    dst->ssid.val[dst->ssid.len] = '\0';

    rt_memcpy(dst->bssid, raw->bssid, RT_WLAN_BSSID_MAX_LENGTH);

    dst->hidden = raw->hidden;
}
        ...
        if(rt_mb_recv(&dev->spi_rx_mb, (rt_ubase_t *)&data_packet, RT_WAITING_FOREVER) == RT_EOK)
        {
            ...
            else if(data_packet->data_type == DATA_TYPE_CB)
            {
                ...
                if(resp->cmd == RT_WLAN_DEV_EVT_SCAN_REPORT)
                {
                    ...
                    parse_scan_data(&wlan_info, (rt_uint8_t *)&resp->value);
...
{
    struct rw007_ap_info_value value = {0};
...
rt_err_t rt_hw_wifi_init(const char *spi_device_name)
{
    ...
    rw007_spi.spi_device = (struct rt_qspi_device *)rt_device_find(spi_device_name);

    if (rw007_spi.spi_device == RT_NULL)
    {
        LOG_E("qspi device %s not found!\r", spi_device_name);
        return -RT_ENOSYS;
    }
    ...
    /* config qspi */
    {
        struct rt_qspi_configuration cfg = {
            .parent.mode       = RT_SPI_MODE_0 | RT_SPI_MSB, /* SPI Compatible: Mode 0. */
            .parent.hard_cs    = 0x00,
            .parent.soft_cs    = 0x80  | RW007_CS_PIN,
            .parent.data_width = 8,
            .parent.max_hz     = RW007_SPI_MAX_HZ,
            .ddr_mode          = 0,
            .medium_size       = 0,
            .qspi_dl_width     = 1,
        };
        ret = rt_qspi_configure(rw007_spi.spi_device, &cfg);
        if (ret != RT_EOK) {
