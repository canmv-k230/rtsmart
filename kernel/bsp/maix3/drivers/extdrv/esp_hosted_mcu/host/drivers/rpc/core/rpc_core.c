                             ...
                             void *argument)
{
    ...
    if (result != RT_EOK)
    {
        rt_thread_delete(g_rpc.event_thread);
        g_rpc.event_thread = RT_NULL;
        goto detach_queue;
    }
...
detach_queue:
    rt_mq_detach(&g_rpc.event_queue);
detach_semaphore:
    rt_sem_detach(&g_rpc.done);
detach_mutex:
    rt_mutex_detach(&g_rpc.mutex);
    rt_memset(&g_rpc, 0, sizeof(g_rpc));
    return result;
}
...
void esp_hosted_rpc_deinit(void)
{
    ...
    g_rpc.initialized = RT_FALSE;
    if (g_rpc.event_thread)
    {
        rt_thread_delete(g_rpc.event_thread);
        g_rpc.event_thread = RT_NULL;
    }
    while (rt_mq_recv(&g_rpc.event_queue, &event, sizeof(event),
