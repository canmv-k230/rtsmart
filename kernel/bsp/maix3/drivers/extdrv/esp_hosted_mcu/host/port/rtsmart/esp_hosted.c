...
#define DBG_TAG "esp_hosted.core"
    ...
    enum esp_hosted_wifi_authmode authmode)
{
    ...
    case ESP_HOSTED_WIFI_AUTH_WPA3_PSK: return SECURITY_WPA3_SAE;
    case ESP_HOSTED_WIFI_AUTH_WPA2_WPA3_PSK:
...
static void eh_wlan_event_thread(void *argument)
{
    ...
    result = rt_wlan_set_mode(g_eh.sta.device.parent.name, RT_WLAN_STATION);
    if (result != RT_EOK)
    {
    ...
    result = rt_wlan_set_mode(g_eh.ap.device.parent.name, RT_WLAN_AP);
    if (result != RT_EOK)
    {
...
int rt_hw_esp_hosted_init(void)
{
    ...
    rt_bool_t state_mutex_initialized = RT_FALSE;
    rt_bool_t event_queue_initialized = RT_FALSE;
    rt_bool_t transport_initialized = RT_FALSE;
    rt_bool_t rpc_initialized = RT_FALSE;
    ...
    state_mutex_initialized = RT_TRUE;
    result = rt_mq_init(&g_eh.wlan_event_queue, "eh-event", g_eh.wlan_event_pool,
                        sizeof(struct eh_wlan_event),
                        sizeof(g_eh.wlan_event_pool), RT_IPC_FLAG_PRIO);
    if (result != RT_EOK)
    {
        goto fail;
    }
    event_queue_initialized = RT_TRUE;
...
fail:
    ...
    if (g_eh.event_thread)
    {
        rt_thread_delete(g_eh.event_thread);
        g_eh.event_thread = RT_NULL;
    }
    esp_hosted_hci_deinit();
    if (rpc_initialized)
    {
        esp_hosted_rpc_deinit();
    }
    if (sta_registered)
    {
        rt_wlan_dev_unregister(&g_eh.sta);
    }
    if (ap_registered)
    {
        rt_wlan_dev_unregister(&g_eh.ap);
    }
    ...
    {
        rt_mq_detach(&g_eh.wlan_event_queue);
    }
    if (state_mutex_initialized)
    {
        rt_mutex_detach(&g_eh.state_mutex);
    }
    rt_memset(&g_eh, 0, sizeof(g_eh));
    g_eh_initializing = RT_FALSE;
    return result;
