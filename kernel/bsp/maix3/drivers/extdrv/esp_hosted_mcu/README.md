...
Enable `RT_USING_ESP_HOSTED_MCU`, then select one transport:
...
Enable `ESP_HOSTED_BLE` to select `RT_USING_BT_HCI`. The controller is
registered as the next available `/dev/hciX` device by the common HCI
framework. The device is a non-blocking
...
existing NimBLE host stack over the selected `/dev/hciX`; NimBLE supplies GAP, GATT, L2CAP,
...
With `ESP_HOSTED_BLE` enabled, an `/dev/hciX` device must also be present and startup must
