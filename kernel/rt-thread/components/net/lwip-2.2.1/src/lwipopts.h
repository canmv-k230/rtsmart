...
#define MEM_CUSTOM_MALLOC(size)     lwip_rt_malloc(size)
#define MEM_CUSTOM_CALLOC(count, size) lwip_rt_calloc((count), (size))
#define MEM_CUSTOM_FREE(mem)        lwip_rt_free(mem)
...
/* Netdev synchronization uses the public lwIP netif extension callback API. */
#define LWIP_NETIF_EXT_STATUS_CALLBACK  1

void *lwip_rt_malloc(size_t size);
void *lwip_rt_calloc(size_t count, size_t size);
void lwip_rt_free(void *mem);
...
/* Preserve the destination address and input netif for recvmsg(IP_PKTINFO). */
